...
public class GuiDemo extends JFrame{
	...
	private JCheckBoxMenuItem gradientOverlayCheckbox = new JCheckBoxMenuItem("Gradient Overlay", false);
	...
	public GuiDemo() {
		...
		drawPanel.getTextItem().setText("'You can't connect the dots looking forward;\nyou can only connect them looking backwards.'\n");
		drawPanel.getTextItem().setFontSize(28);
		drawPanel.getTextItem().setJustify(TextItem.CENTER);
		drawPanel.getTextItem().setItalic(true);
		drawPanel.getTextItem().setFontName("Inter");
		drawPanel.getTextItem().setBold(true);
		drawPanel.getTextItem().setColor(Color.BLACK);


		drawPanel.setBackgroundImage(Util.getImageResource("resources/images/applepark.jpeg"));
	...
	private JToolBar makeToolbar() {
		...
		newPict.setToolTipText("Clear picture to default state.");
		JButton saveImg = tbar.add(saveImageAction);
		saveImg.setToolTipText("Save Image");


		JButton addImage = tbar.add(new ChooseBackgroundAction("Custom..."));

		addImage.setIcon(Util.iconFromResource("resources/icons/addimage.png"));
		JButton colorImage = tbar.add(new ChooseBackgroundAction("Color..."));
		tbar.addSeparator(new Dimension(16, 0));
		...
		tbar.add(new ChooseBackgroundAction("applepark"));
	...
	private JMenu makeBackgroundMenu() {
		...
		menu.add(new ChooseBackgroundAction("applepark"));
		menu.addSeparator();
    ...
    private class ChooseBackgroundAction extends AbstractAction {
    	...
    	ChooseBackgroundAction(String text) {
    		...
    		if (text.equals("Color...")) {
				...
				BufferedImage colorSel = new BufferedImage(32, 32, BufferedImage.TYPE_INT_RGB);Graphics g = colorSel.createGraphics();g.setColor(Color.RED);
				g.fillRect(0, 0, 10, 32);
				g.setColor(Color.GREEN);
				g.fillRect(11, 0, 11, 32);
				g.setColor(Color.BLUE);
				g.fillRect(22, 0, 10, 32);
				g.dispose();putValue(Action.SMALL_ICON, new ImageIcon(colorSel));
			}

    		else if (text.equals("Custom...")) {
    			putValue(Action.SHORT_DESCRIPTION, "<html>Select an image file<br>to use as the background.</html>");
				putValue(SMALL_ICON, Util.iconFromResource("resources/icons/addimage.png"));
			}
